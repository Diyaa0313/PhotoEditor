onload = function () {
  ...
  const tools = {
    ...
    sepia: function () {
      ...
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          ...
          image[i][j][0] = Math.min(255, r);
          image[i][j][1] = Math.min(255, g);
          image[i][j][2] = Math.min(255, b);
        }
      }
      setImageData(image, rows, cols);
    },
    ...
    contrast: function () {
      ...
      const contrastFactor = 2; // You can adjust this factor based on your preference

      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          for (let k = 0; k < 3; k++) {
            image[i][j][k] = (image[i][j][k] - 128) * contrastFactor + 128;
          }
        }
      }
      setImageData(image, rows, cols);
    ...
    saturation: function () {
      ...
      const saturationFactor = 1.5; // You can adjust this factor based on your preference

      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          const pixel = image[i][j];
          const grayscale = 0.3 * pixel[0] + 0.59 * pixel[1] + 0.11 * pixel[2];

          for (let k = 0; k < 3; k++) {
            image[i][j][k] =
              grayscale + saturationFactor * (pixel[k] - grayscale);
          }
        }
      }
      setImageData(image, rows, cols);
    ...
    applyFilter: function (kernel) {
      let cols = editor.width;
      let rows = editor.height;
      let image = getRGBArray(rows, cols);
      applyFilter(image, rows, cols, kernel);
    },
  };
